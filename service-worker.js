const CACHE = "diktat-player-local-package-v3";
const GENERATED_CACHE = "diktat-generated-files-local-package";
...
self.addEventListener("activate", event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(key => key.startsWith("diktat-player-local-package-") && key !== CACHE && key !== GENERATED_CACHE).map(key => caches.delete(key))))
